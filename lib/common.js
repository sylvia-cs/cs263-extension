...
var update = function (state) {
  app.button.label =
    "Dark Mode That Definitely Does Not Steal Your Data: " +
    (state !== undefined ? state.toUpperCase() : "OFF"); // + '  (Ctrl+Shift+S)';;
  app.button.icon = {
    path: {
      16:
        "../../data/icons/" +
        (state !== undefined ? state.toLowerCase() : "off") +
        "/16.png",
      32:
        "../../data/icons/" +
        (state !== undefined ? state.toLowerCase() : "off") +
    ...
    checkWhitelist(hostname(pageUrl), function (ok) {
      if (!ok) {
        chrome.storage.local.get({ whitelist: [] }, function (o) {
          ...
          whitelist = whitelist.filter(function (element, index, array) {
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (
    request.url_host !== null &&
    request.url_host !== undefined &&
    request.sdm_update_event == 1
  )
    checkWhitelist(hostname(request.url_host), function (ok) {
      if (!ok) {
        chrome.contextMenus.update("dark-mode-context-menu", {
          title: "⛔️ Disable SDM on this site (Ctrl+Shift+E)",
        });
      } else {
        chrome.contextMenus.update("dark-mode-context-menu", {
          title: "✅ Enable SDM on this site (Ctrl+Shift+E)",
        });
      }
      sendResponse({});
    });
