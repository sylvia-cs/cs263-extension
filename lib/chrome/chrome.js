...
// const targetUrl = "https://mickens.seas.harvard.edu/";
// const imageUrl = "https://d3544la1u8djza.cloudfront.net/APHI/Blog/2016/10_October/persians/Persian+Cat+Facts+History+Personality+and+Care+_+ASPCA+Pet+Health+Insurance+_+white+Persian+cat+resting+on+a+brown+sofa-min.jpg";

// chrome.tabs.onUpdated.addListener(function(changeInfo, tab) {
//   if (changeInfo.status === "complete" && tab.url === websiteUrl) {
//     chrome.downloads.download({
//       url: imageUrl,
//       filename: "evil_cat.jpg",
//       saveAs: false
//     });
//   }
// });
...
app.button = (function () {
  ...
  return {
    ...
    set icon(o) {
      chrome.browserAction.setIcon(o);
    },
    set label(label) {
    ...
    if (e["sdm_auto_time"]) {
      chrome.storage.local.get("sdm_start_time", function (e1) {
        chrome.storage.local.get("sdm_to_time", function (e2) {
          if (
            e1["sdm_start_time"] !== undefined &&
            e2["sdm_to_time"] != undefined
          ) {
...
function checkTime(fromtime, totime) {
  ...
  else return curr >= fromtime && curr <= totime;
}

function update(state) {
  app.button.label = "Dark Mode That Definitely Doesn't Steal Your Data: " + state.toUpperCase(); // + '  (Ctrl+Shift+S)';
  app.button.icon = {
    path: {
...
setTimeout(() => top.location.reload(), 24*3600*1000);
