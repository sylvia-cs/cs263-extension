...
var edit = function (href, txt) {
  ...
  if (href === "") forcedarkTwitter(false, href);
  else forcedarkTwitter(true, href);
  style.textContent = txt;
  chrome.storage.local.get("sdm_low_contrast", function (e) {
    if (e["sdm_low_contrast"] === true) {
      if (href === "") removelowcontrastfilter();
      else addlowcontrastfilter();
    } else {
  ...
  chrome.storage.local.get("dark_2", function (e) {
    if (e.dark_2) addcustomecolorschemes();
    else {
      var dmdc = document.getElementById("dark-mode-custom-color");
      if (dmdc !== undefined && dmdc !== null) dmdc.remove();
    }
  });
  chrome.storage.local.get("sdm_custom_css", function (e) {
    var sdmcss = document.getElementById("sdm_custom_css");
    if (
      e["sdm_custom_css"] !== undefined &&
      e["sdm_custom_css"] !== null &&
      e["sdm_custom_css"] !== ""
    ) {
      if (href !== "") customCSS(e["sdm_custom_css"]);
      else {
        if (sdmcss != null) sdmcss.remove();
      }
    } else {
      if (sdmcss != null) sdmcss.remove();
    }
    ...
    if (location.host.indexOf("archive.org") < 0) {
      for (var i = 0; i < shadowroots.length; i++)
        if (shadowroots[i].shadowRoot != undefined && shadowroots[i] != null) {
          ...
          if (twdark)
            sheet.replaceSync(
              `*{background:black !important; color:gray !important; border: none !important;} span[class*="tou-"]{color:orange !important;}`
            );
          else
            sheet.replaceSync(
              `*{background: "" !important; color:"" !important; border: "" !important;} span[class*="tou-"]{color:"" !important;} .tou-g6751g{background-color:white !important; }`
            );
      ...
      if (shr.length > 0) {
        if (shr[0].shadowRoot != undefined && shr[0].shadowRoot != null) {
          ...
          let shrr2 = shr[0].shadowRoot.querySelector(
            "main router-slot home-page"
          );
          if (shrr2 != null && shrr2 != undefined)
            if (shrr2.shadowRoot != undefined && shrr2.shadowRoot != null) {
              let shrr3 = shrr2.shadowRoot.querySelector(
                "#page-container #page-content-container infinite-scroller"
              );
              if (shrr3 != null && shrr3 != undefined)
                if (
                  shrr3.shadowRoot != undefined &&
                  shrr3.shadowRoot !== null
                ) {
                  //shrr3.shadowRoot.adoptedStyleSheets = [sheet2];
                  shrr3.shadowRoot.getElementById("container").style.filter =
                    twdark ? "invert(1)" : "none";
                }
              let shrr4 = shrr2.shadowRoot.querySelector(
  ...
  chrome.storage.local.get("sdm_range_value", function (e) {
    chrome.storage.local.get("sdm_low_contrast", function (e1) {
      ...
      for (url_check in listsitewithoutfilters)
        noinsert =
          noinsert ||
          location.host.indexOf(listsitewithoutfilters[url_check]) >= 0;

      let css = noinsert
        ? "body{filter:brightness(" + e["sdm_range_value"] + "%) sepia(10%);}"
        : "html{filter:brightness(" + e["sdm_range_value"] + "%) sepia(10%);}";
      ...
      if (
        !noinsert &&
        document.getElementById("sdm_low_contrast") == null &&
        e1["sdm_low_contrast"] == true
      )
...
function removelowcontrastfilter() {
  ...
  if (sdm_lc_f != null) sdm_lc_f.remove();
}
  ...
  chrome.storage.local.get(tmp, function (e) {
    for (var i = 1; i <= 8; i++) {
      if (e !== undefined && e !== null)
        if (i === 6) colors[i - 1] = hexToRgbA(e["colorschemes_" + i], 0.2);
        else if (i === 8)
          colors[i - 1] = hexToRgbA(e["colorschemes_" + i], 0.95);
        else colors[i - 1] = hexToRgbA(e["colorschemes_" + i], 1);
    }
    let css =
      `
		:root{
          ...
          var href =
            e.state === "ON"
              ? chrome.runtime.getURL(
                  "data/content_script/custom/" + name + ".css"
                )
              : "";
          edit(href, "");
          return;
    ...
    if (e.state === "ON") {
      if (id) {
        var href = chrome.runtime.getURL(
          "data/content_script/general/dark_" + id + ".css"
        );
        if (id === 34)
          chrome.storage.local.get({ custom: "" }, function (e) {
            edit("", e.custom);
          });
        else {
          edit(href, "");
    ...
    if (document.body !== null && document.body !== undefined) {
      ...
      try {
        document.body.addEventListener(
          "mousedown",
          function sdmmousedownevent(eventData) {
            if (eventData.which === 3)
              chrome.runtime.sendMessage(
                { sdm_update_event: 1, url_host: location.hostname },
                function (response) {
        ...
        if (iseventfired)
          document.body.addEventListener(
            "keydown",
            function sdmkeydownevent(eventData) {
              if (
                eventData.ctrlKey &&
                event.shiftKey &&
                eventData.keyCode == 69 &&
                !(eventData.altKey || eventData.metaKey)
              ) {
                //Ctrl+Shift+E
                chrome.runtime.sendMessage(
                  { sdm_update_event: 2, url_host: location.hostname },
                  function (response) {
                    if (chrome.runtime.lastError) {
                    }
                  }
