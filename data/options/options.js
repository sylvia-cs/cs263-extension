...
var handle = function () {
  ...
  if (id.indexOf("dark_") !== -1) {
    for (var i = 1; i <= n_dark_themes; i++)
      document.getElementById("dark_" + i).checked = false;
    document.getElementById(id).checked = checked;
  }
  if (document.getElementById("dark_2").checked === true) {
    document.getElementById("custom_color_scheme").style.display = "";
  } else document.getElementById("custom_color_scheme").style.display = "none";
  var tmp = {};
  for (var name in custom) tmp[name] = document.getElementById(name).checked;
  for (var i = 1; i <= n_dark_themes; i++)
    tmp["dark_" + i] = document.getElementById("dark_" + i).checked;
  for (var i = 1; i <= 8; i++)
    tmp["colorschemes_" + i] = document.getElementById(
      "customcolor_" + i
    ).value;
...
var restore = function () {
  ...
  for (var i = 1; i <= 8; i++) {
    document.getElementById("customcolor_" + i).value = colors[i - 1];
  }
  ...
  chrome.storage.local.get(tmp, function (e) {
    ...
    for (var i = 1; i <= n_dark_themes; i++)
      document.getElementById("dark_" + i).checked = e["dark_" + i];
    for (var i = 1; i <= 8; i++) {
      if (e["colorschemes_" + i] !== undefined)
        document.getElementById("customcolor_" + i).value =
          e["colorschemes_" + i];
    }
    if (document.getElementById("dark_2").checked === true)
      document.getElementById("custom_color_scheme").style.display = "";
...
var load = function () {
  ...
  for (var name in custom)
    document.getElementById(name).addEventListener("click", handle);
  for (var i = 1; i <= n_dark_themes; i++)
    document.getElementById("dark_" + i).addEventListener("click", handle);
  for (var i = 1; i <= 8; i++)
    document
      .getElementById("customcolor_" + i)
  ...
  document.getElementById("whitelist").addEventListener("change", function () {
    ...
    for (var i = 0; i < hosts.length; i++)
      whitelist_tmp.push(hostname(hosts[i]));
    whitelist_tmp = whitelist_tmp.filter(function (element, index, array) {
  ...
  chrome.storage.local.get("sdm_to_time", function (e) {
    if (e["sdm_to_time"] !== undefined) to_time.value = e["sdm_to_time"];
  });
  ...
  chrome.storage.local.get("option_page_tab_active", function (e) {
    if (
      e["option_page_tab_active"] !== undefined &&
      e["option_page_tab_active"] !== null
    ) {
...
enablepattern.addEventListener("change", function () {
  chrome.storage.local.set(
    { sdm_enable_pattern: this.checked },
    function () {}
  );
});
...
document.getElementById("btresetcolor").addEventListener("click", function () {
  ...
  chrome.storage.local.set(tmp, function () {});
  setTimeout(function () {
    document.getElementById("btresetcolor").value = "Reset to Default";
  }, 1000);
});
...
vpillshometab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-home-tab" },
    function () {}
  );
};
vpillsautomodetab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-automode-tab" },
    function () {}
  );
};
vpillsspthemetab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-sptheme-tab" },
    function () {}
  );
};
vpillsadsettingstab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-adsettings-tab" },
    function () {}
  );
};
vpillswhilelisttab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-whilelist-tab" },
    function () {}
  );
};
vpillscustomcsstab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-customcss-tab" },
    function () {}
  );
};
vpillssponsoringtab.onclick = function () {
  chrome.storage.local.set(
    { option_page_tab_active: "v-pills-sponsoring-tab" },
    function () {}
  );
};
